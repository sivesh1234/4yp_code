...
#Getting data
vod = pd.read_csv("fxData.csv")#year, month, date
short_window = 41
long_window = 101

#gives signals the same index (dates) as vod
vod['signal'] = 0.0
vod['short_mavg'] = vod['USDJPY'].rolling(window=short_window,
                                              min_periods=1,center=False).mean()
vod['long_mavg'] = vod['USDJPY'].rolling(window=long_window,min_periods=1,center=False).mean()
...
last3000 = vod['USDJPY'][-3000:]
open_price = vod['USDJPY'][0]
...
features_considered = ['signal','USDJPY']
...
future_target = 5 #periods predicted for testing
