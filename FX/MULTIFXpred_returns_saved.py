...
def predicted_returns(history, true_future, prediction):
  ...
  if prediction_average > 0.9825:
      signal = 1
      print("long")
  elif prediction_average < 0.0175:
      signal = -1
      print("short")
  else:
      signal = 0
      number_of_flat = number_of_flat +1
...
predictions_array = load('USDJPY_predictions.npy')
for alpha in range(0,5000,trade_horizon):
    ...
    signal,returns, pred = predicted_returns(x_val_multi[alpha],y_val_all[alpha],predictions_array[trade])
...
print("number of flat trades is {}".format(number_of_flat))
print("average trade return is {}".format(total_returns/(1000-number_of_flat)))
plt.show()
