...
#Getting data
tar_data = pdr.get_data_yahoo('VOD',
...
C_data = pdr.get_data_yahoo('BT-A.L',
                          start=datetime.datetime(1990, 10, 26),
                          end=datetime.datetime(2019, 10, 26))                          #year, month, date
...
def multi_step_plot(history, true_future, prediction,signal,returns):
...
model = tf.keras.models.load_model('saved_model/VOD_multi1-0_model')
...
for alpha in range(0,3000,30):
    ...
    # multi_step_plot(x_val_multi[alpha],y_val_all[alpha],pred,signal,returns)
...
plt.title('VODmulti-PnL using RNN 1 or 0 prediction (threshold)')
plt.show()
