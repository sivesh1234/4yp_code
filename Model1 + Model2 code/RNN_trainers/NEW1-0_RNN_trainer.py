...
tar_data['pct_change'] = tar_data['Close'].pct_change()
tar_data['pct_change_average'] = tar_data['pct_change'].rolling(window=short_window,
                                              min_periods=1,center=False).mean()
...
x_train_multi, y_train_multi = multivariate_data(dataset[:,1], dataset[:, 0], 0,
                                                 TRAIN_SPLIT, past_history,
                                                 future_target, STEP)
 #Returns testing data set = x and labels = y
x_val_multi, y_val_multi = multivariate_data(dataset[:,1], dataset[:, 0],
...
plot_train_history(multi_step_history, 'Single stock model training and validation loss')


#Save model

model.save('saved_model/NEW1-0_model')
